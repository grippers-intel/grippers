...
| **먼 쪽 물체만 검출 안 됨** | C270가 1280×720으로 열렸는지 확인 — 권고 50 mm 물체는 최악점에서도 26.7 px라 정상이면 잡힌다. **최대 폭이 37 mm 미만이면 규격 문제**. YOLO resize(`imgsz`)도 확인 |
| **가까운 벽 앞 물체만 검출 안 됨** | 정상이다 — 그 카메라의 실효 커버는 벽에서 **0.21 / 0.26 m부터** 시작한다(화각 전폭). 그 띠는 **마주보는 반대편 카메라**가 덮으므로 **두 대가 다 열렸는지** 확인 |
...
| **체스말을 가베 상자에 넣음** | 체스말 베이스가 원형인지 확인 — **각진 베이스로 재프린팅.** 카메라 기울임 각도 재확인 |
