...
| **먼 쪽 물체만 검출 안 됨** | 웹캠 해상도 확인 — **1080p 미만이면 원인 확정.** 광각 렌즈면 화각 축소 |
| **파지가 일정한 방향으로 빗나감** | 호모그래피에 **박스 중심**을 넣고 있는지 확인 → **아래쪽 모서리**로 교체 |
| **검정 상자를 못 찾거나 그림자를 상자로 오인** | 밝은 테두리·ArUco 부착 여부 확인. V 임계값만으로는 그림자와 구분 불가 |
| 상자 색 인식 실패 | 화이트밸런스 고정 여부, HSV 범위 재튜닝 |
