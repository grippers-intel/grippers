...
| **먼 쪽 물체만 검출 안 됨** | C270가 1280×720으로 열렸는지 확인 — `toy` 38~40 mm면 최악점 20.3~21.4 px라 정상이면 잡힌다. **여유가 7%뿐이라** 조명·모션블러부터 의심할 것. **최대 폭 37.4 mm 미만이면 규격 문제**. YOLO resize(`imgsz`)도 확인 |
