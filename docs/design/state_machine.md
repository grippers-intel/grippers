...
> **2026-09-04 사용자 지시로 위치 판정(라이다)을 뺐다.** Host가 좌표·경로·정지 위치를
> 전부 소유한다는 원칙을 INSERT까지 끝까지 적용한 것이다 — "정말 도착했는가"를 Pi가
> 라이다로 재확인하던 이중 판정을 없앴다. 아래 조건은 전부 "지금 팔을 펼쳐도 안전한가"이지
> "바구니가 정말 거기 있는가"가 아니다. 위치가 틀렸을 때의 안전판은 이제 Host 쪽 ArUco
> 하드스톱(`mission_config.BASKET_HARD_STOP_MARGIN_M`)뿐이다 — Pi는 더 이상 걸러내지
> 않는다. 라이다 기반 판정이었던 옛 버전은 이 문서의 git 이력 참고.
...
- 무엇을 들고 있는지 안다(`plan_for_label`이 프로필을 냄) — 놓기 폭을 정할 수 있어야 한다
- 직전 사이클 대비 부하 하락이 허용치 안(`GRIPPER_SLIP_LOAD_DROP`) — 미끄러짐 감지, 라이다와
  무관한 독립 신호라 위치 판정을 빼도 그대로 남는다

`APPROACH_BOX` 접근도 이제 `CARRY`와 완전히 같은 경로를 탄다 — Host가 보낸 속도를 그대로
낼 뿐, Pi가 거리를 재서 끼어들지 않는다(2026-09-02~09-04까지는 매 사이클 라이다를 봐서
너무 가까우면 물러나고 목표창 안이면 먼저 멈췄었다).
