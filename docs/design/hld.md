...
⚠️ **INSERT는 더 이상 Pi 로컬 판정이 아니다** — 2026-09-04부터 위치를 전적으로 Host가
정하고 Pi는 자기 상태(정지·파지·부하)만 본다. `Lidar.basket_face`는 여전히 존재하지만
INSERT 판정에서 더는 호출되지 않는다.
...
이전 판 §3 "보류" 표기). 2026-09-03까지 INSERT는 기하 해석 대신 라이다 정면 판정의
문턱값 비교로 위치를 확인했지만, **2026-09-04부터는 그 확인 자체가 없다** — Host가
INSERT를 보내면 Pi는 자기 상태(정지·파지·부하)만 보고 그대로 따른다. 알고리즘이랄
것도, 위치 문턱값 비교랄 것도 이제 없다.
