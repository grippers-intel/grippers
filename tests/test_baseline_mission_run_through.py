...
def _load_script():
    ...
    ⚠️ 2026-09-03 실기(box, 3번째 시도)로 들어올리는 과정에서의 부하
    판독을 전부 없앴다("닫은 직후" 판독, "midpoint 유지 확인" 재확인 둘
    다) — 서보가 목표 자세에 도달해 정착하면 실제로 여전히 물고 있어도
    ...
    return [
        ...
        HOLDING,    # 2. CARRY 표본 1
        HOLDING,    # 3. CARRY 표본 2
        HOLDING,    # 4. INSERT 판정 사이클의 표본
        HOLDING,    # 5. 투하 직전
        EMPTY,      # 6. 투하 직후 — 손을 떠났다
    ]
