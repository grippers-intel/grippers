...
def test_교시_각도제한이_백업_파일과_같다():
    ...
    #                 나왔는데 지금은 안 나온다."
    #
    #   1007 -> 1090  **실기가 되돌렸다**(2026-09-08). 배포한 브랜치가 1007 을
    #                 기대하는데 하드웨어는 1090 이라 arm_driver 가
    #                 ArmCalibrationMismatchError 로 죽었다. 팀 공용 EEPROM 은
    #                 팀원 브랜치가 09-07 에 1090 으로 잡았고 하루 종일 그 값으로
    #                 돌았다 — 사용자도 "VLA 기반에서는 물체를 딱히 떨어뜨린 적이
    #                 없다"고 했는데 그 파지가 전부 1090 에서 났다. 그래서
    #                 하드웨어를 코드에 맞추는 대신 코드를 하드웨어에 맞췄다.
    #                 1007 은 servo 6 이 과전류로 버스에서 떨어지던 설정이기도
    #                 하다(온도 50°C, 다른 서보 32~35°C) — 그 문제는 미해결이다.
    #
    # 1007 자체는 지어낸 값이 아니라 텔레옵이 실제로 쓰던 자리다. lerobot
    # 캘리브레이션의 그리퍼 range_min=1960 을 지금 프레임으로 옮긴 값이고
    # (390 - 1343 = -953), 텔레옵이 그 하한으로 몇 달을 돌았다. 무는 힘을 다시
    # 깊게 하려면 gripper_grip_gain.py --restore-teleop-grip 로 **하드웨어와
    # 함께** 1007 로 옮겨야 한다 — 한쪽만 바꾸면 arm_driver 가 또 죽는다.
    #
