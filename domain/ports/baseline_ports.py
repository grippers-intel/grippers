...
class MissionState:
    ...
    # 지시) — Pi의 BaselineInsertState가 "drop" 자세 도달 후 그리퍼를 열기
    # **전에** command.yaw_correction_deg로 servo 1을 보정할 때만 한 차례
...
class HostCommand:
    ...
    # 서게 하고, 남은 오차는 팔로 보정한다. angular_z와 마찬가지로 Host가
    # 계산해 값만 건네고 Pi는 그 값이 어디서 왔는지 모른 채 실행만 한다.
    # 0.0이면 보정 없음(safe_300 단계 자체를 건너뛴다 — grippers-baseline-wt의
    # BaselineInsertState 참고. 이 필드는 두 저장소의 baseline_ports.py에
    # 각각 독립적으로 유지된다 — 공유 git 이력이 없다).
