...
# 배터리 8.4V):
#
#     명령 0.05  실측 0.015 m/s  손실 70%   <- 데드밴드 근처
#     명령 0.10  실측 0.088 m/s  손실 12%
#     명령 0.15  실측 0.129 m/s  손실 14%
#     명령 0.20  실측 0.186 m/s  손실  7%
#     명령 0.25  실측 0.223 m/s  손실 11%
#
# 같은 날 7.4V 에서는 0.25 까지 올려도 안 나갔다. 전압이 떨어지면 데드밴드가
# 올라가므로, 만충 기준으로 겨우 넘는 값을 쓰면 시연 도중에 멈춘다.
# 0.15 는 실측 최소(0.10)의 1.5배다.
#
# ⚠️ 라이다 빔 변화로는 이 값을 못 잰다. 평행이동에 둔감해서 실제로 70mm 를
# 가도 "안 움직였다"로 찍힌다(tools/linear_threshold_sweep.py 가 그랬다).
# 탑뷰 ArUco 로 재야 한다.
