...
# ⚠️ 0.1 은 만충에서만 여유가 있다. 2026-09-06 탑뷰 ArUco(mm) 실측:
#
#   8.4V  0.05 -> 0.015 m/s(손실 70%)  0.10 -> 0.088(12%)  0.15 -> 0.129(14%)
#   8.0V  0.10 -> 안 움직임
#   7.4V  0.25 까지 올려도 안 움직임
#
# 데드밴드가 전압을 따라 올라간다. 만충 기준으로 겨우 넘는 값을 쓰면 시연
# 도중에 멈춘다 — 8.4V 에서 되던 0.1 이 8.0V 에서 안 됐다.
AGREED_LINEAR_MPS = 0.15
# ⚠️ 회전에도 데드밴드가 있다. 2026-09-05·06 IMU 실측에서 8.2V·7.4V 둘 다
# 0.25 는 제자리에서 안 돌았고(실측 0.035~0.049 rad/s) 0.35 부터 돌았다.
# 2026-08-24 기록의 0.355 와 세 번째로 일치한다. 만충(8.4V)에서는 0.25 도
# 돌지만 전압이 떨어지면 못 돈다 — 시연 중 방전을 견뎌야 한다.
#
# 0.6 은 실측 최소(0.35)의 1.7배다. 실제 회전은 약 20도/s 이고 Host 루프가
# 10Hz 이므로 사이클당 2도인데, DRIVE_YAW_TOLERANCE_DEG 는 12도다.
