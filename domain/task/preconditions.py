...
class InsertInputs:
    ...
    base_stopped: bool
    gripper_load: float
    # ⚠️ 2026-09-08 사용자 지시로 grasp_confirmed 를 지웠다:
    #
    #   "진짜 쥐었는지 확인하는 단계와 로봇과 노트북이 집은 것과 못 집은
    #    것에 대해 양방향 소통하는 것이 필요없는 거 같아"
    #
    # 그 값은 GRASP 가 내린 파지 판정을 INSERT 까지 들고 오던 것인데,
    # 파지 판정 자체가 없어졌으므로(baseline_mission 의 그 주석) 항상 True
    # 였다. 죽은 인자가 "여기서 뭔가 걸러진다"처럼 읽히는 것이 더 나쁘다.
    #
    # 빈손으로 투하 자세를 펴는 것은 무해하고, 물체가 아직 바닥에 있는지는
    # 탑뷰가 이미 안다 — 다음 사이클의 SEARCH_TARGET 이 그 기물을 다시 찾는다.
    face_ok: bool
