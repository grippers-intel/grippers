...
HANDOFF.md가 검증했지만, 이 회전 버전은 처음이다). 세 상수 중 MAX_TURN·
MIN_TURN은 2026-08-24 실측으로 확정했고(아래 주석 참고), GAIN_TURN만
아직 자리 표시자다 — 실기에서 가장 먼저 확인할 것.
...
# ✅ 2026-08-24 실기 실측으로 두 회전 상수는 자리표시자를 벗어났다
# (tools/inplace_rotation_test.py — 1.2~0.3 rad/s 전부 회전 확인, 문턱은
# 0.3보다 낮다). scan_track_control.py의 같은 날짜 주석에 근거가 있다.
# 값은 사용자가 실기를 보며 정한 정렬용 범위 0.3~0.4를 따른다.
DEFAULT_MAX_TURN = 0.4  # rad/s
# 이 아래로는 아예 안 낸다 — 예전 값 0.15는 "정지마찰을 못 이길 것"이라는
# **추정**이었는데, 실측이 0.3에서 도는 것을 확인했으므로 실제로 도는 것이
# 확인된 가장 낮은 값을 쓴다. 0.15는 확인된 적이 없는 값이라 그대로 두면
# 오차가 작을 때 로봇을 못 돌리는 명령을 내게 된다.
DEFAULT_MIN_TURN = 0.3  # rad/s
