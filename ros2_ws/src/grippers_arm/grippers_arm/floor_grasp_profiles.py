...
TAUGHT_POSITION_LIMITS = {
    ...
    # 반대 방향이다.
    #
    # ⚠️ 팀 공용 하드웨어의 EEPROM 이다. 되돌리려면
    # `tools/arm/restore_taught_offsets.py --apply --yes` 인데, 그 도구는
    # 이제 **이 값(1007)** 으로 복원한다는 점에 주의. 무는 힘만 되돌리려면
    # `tools/arm/gripper_grip_gain.py --set-min-limit 1090`.
    6: (1007, 2090),    # gripper — 2026-09-01 사고가 난 바로 그 레지스터
}
...
#
# 사용자: "파지 후에 주행으로 넘어가는 기본주행 자세에서 그리퍼의 각도가
# 달라서 자꾸 기물을 떨어뜨리고 있어."
#
