...
# 2026-09-05 reteach_idle_pose.py 실측. 팔을 손으로 크래들에 앉히고 확정했다.
#
# ⚠️ 08-30 의 영점 이동 보정(76f0ef5)이 이 자세들을 옮겼는데, 같은 커밋이
# TAUGHT_HOMING_OFFSETS 는 안 바꿨고 서보에는 지금도 그 교시 오프셋이 그대로
# 들어 있다(restore_taught_offsets.py 로 확인). 옮길 이유가 없는데 옮긴 것이다.
# wrist_roll 이 특히 크게 어긋났다 — 실측 3023 대비 옮긴 값은 1022 차이인데
# 옮기기 전 값(3071)은 48 차이다. 그 1022 는 VLA 캘리브레이션과 교시
# 캘리브레이션의 프레임 차이(976)와 거의 같다.
#
# 이 어긋남 때문에 arm_driver 가 "등록된 자세 어디에도 가깝지 않다"며 모든
# 이동을 거부했고(2026-09-05 실기), 정렬을 걸면 손목을 90도 돌렸다.
IDLE_CRADLE_RAW = (2096, 822, 3101, 2949, 3023)
...
# IDLE 에서 servo 4(손목)만 들어올린다. 옮기기 전 표의 관계(2751 -> 2514,
# 즉 -237)를 실측 IDLE 에 그대로 적용했다. 지금까지 쓰던 -379 는 IDLE 만
# 손으로 다시 잡고 CARRY 는 계산으로 옮기면서 생긴 값이라 설계 의도가 아니다.
