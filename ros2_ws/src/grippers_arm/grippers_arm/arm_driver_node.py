...
class ArmDriverNode(Node):
    ...
    # 없다.
    #
    # ⚠️ 60도는 **45도까지만 실기로 검증됐다** — 45도 이내(6.2도·25.3도
    # 실측)에서는 팔이 차체·배선에 닿지 않고 두 번 다 성공했지만, 45도를
    # 넘는 구간(46~60도)은 아직 실기로 확인된 적이 없다. 사용자가 위험을
    # 감수하고 확장 지시한 수치다 — 처음 45도를 넘는 각으로 시험할 때
    # 팔이 차체 옆이나 배선에 닿지 않는지 반드시 눈으로 확인할 것.
    MAX_DROP_YAW_OFFSET_RAD = math.radians(60.0)
