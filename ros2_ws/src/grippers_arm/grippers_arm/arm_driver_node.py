...
class ArmDriverNode(Node):
    ...
    # 아슬아슬"한 경우가 나왔다 — 사용자 지시로 45도로 다시 낮췄다(Host
    # 쪽 basket_target.NO_ROTATION_* 로 "이미 충분히 좋은 자세면 아예 안
    # 돌린다"를 새로 둔 것과 같은 세션, 같은 문제에 대한 대응).
    ...
    # 없다.
    #
    # ⚠️ 45도 이내(6.2도·25.3도 실측)에서는 팔이 차체·배선에 닿지 않고
    # 두 번 다 성공했다 — 그 이상(46도~)은 실기로 검증된 적이 없으니
    # 다시 넓히기 전에 반드시 눈으로 확인할 것.
