...
class ArmDriverNode(Node):
    ...
    # 사용자 지시로 20도까지 더 낮췄다 — "servo1 보정을 최소한으로 해볼까"
    # (회전 자체를 최소화하는 쪽으로 계속 좁히는 방향). Host 쪽
    # basket_target.NO_ROTATION_* (이미 충분히 좋은 자세면 아예 안 돌린다)
    # 와 같은 문제에 대한 같은 방향의 대응이다.
    ...
    # 없다.
    #
    # ⚠️ 20도는 그 자체로 실기 검증된 적은 없다 — 다만 이미 검증된 6.2도·
    # 25.3도 실측 중 20도 밑인 6.2도는 그대로 포함되므로 완전히 새 영역은
    # 아니다. 20도를 넘던 보정(예: 22.6도)은 이제 아예 거부되어 servo1이
    # 안 움직이고 드랍한다 — 그 결과(중앙에서 더 벗어난 위치에 놓이는지)를
    # 실기로 확인할 것.
    MAX_DROP_YAW_OFFSET_RAD = math.radians(20.0)
