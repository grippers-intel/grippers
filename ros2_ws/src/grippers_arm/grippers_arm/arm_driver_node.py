...
from grippers_interfaces.srv import SetGripper, GetLoad
...
class ArmDriverNode(Node):
    def __init__(self):
        ...
        self._move_action_server = ActionServer(
            self, MoveToCartesian, 'arm_driver/move_to_cartesian',
            execute_callback=self._execute_move,
            callback_group=cb_group,
        )
        self.create_service(
            SetGripper, 'arm_driver/set_gripper',
            self._on_set_gripper, callback_group=cb_group,
        )
        self.create_service(
            GetLoad, 'arm_driver/get_load',
            self._on_get_load, callback_group=cb_group,
        )
    ...
    def _execute_move(self, goal_handle):
        ...
        try:
            angles_deg, err = soarm.arm.go(
                xyz, grip=grip, real=True, down=req.down, secs=1.2,
            )
