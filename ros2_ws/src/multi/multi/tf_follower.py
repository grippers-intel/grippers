...
class TFFollower(Node):
    def __init__(self):
        ...
        self.leader_frame = (
            self.get_parameter("leader_frame").get_parameter_value().string_value
        )
        self.follower_frame = (
            self.get_parameter("follower_frame").get_parameter_value().string_value
        )
        self.offset_x = (
    ...
    def timer_callback(self):
        try:
            ...
            leader_transform = self.tf_buffer.lookup_transform(
                "odom", self.leader_frame, rclpy.time.Time()  # 目标帧  # 源帧
            )  # 当前时间
            ...
            follower_transform.transform.translation.x = (
                leader_transform.transform.translation.x + dx_world
            )
            follower_transform.transform.translation.y = (
                leader_transform.transform.translation.y + dy_world
            )
            follower_transform.transform.translation.z = (
                leader_transform.transform.translation.z + dz_world
            )
