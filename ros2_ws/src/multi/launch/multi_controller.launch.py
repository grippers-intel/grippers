...
def generate_launch_description():
    ...
    master_arg = DeclareLaunchArgument(
        "master", default_value=EnvironmentVariable("MASTER")
    )
    robot_name_arg = DeclareLaunchArgument(
        "robot_name", default_value=EnvironmentVariable("HOST")
    )
    cmd_vel_topic_arg = DeclareLaunchArgument(
        "cmd_vel_topic", default_value="/controller/cmd_vel"
    )
    ...
    joy_control_include = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(
                get_package_share_directory("multi"), "launch", "joy_control.launch.py"
            )
        ),
        condition=IfCondition(
            PythonExpression(['"', master, '" == "', robot_name, '"'])
        ),
    )
