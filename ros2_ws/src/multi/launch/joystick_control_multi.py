...
class JoystickController(Node):
    def __init__(self, name, num_robots):
        ...
        for i in range(1, num_robots + 1):
            ...
            robot = {
                "servo_state_pub": self.create_publisher(
                    SetPWMServoState,
                    robot_namespace + "ros_robot_controller/pwm_servo/set_state",
                    1,
                ),
                "buzzer_pub": self.create_publisher(
                    BuzzerState, robot_namespace + "ros_robot_controller/set_buzzer", 1
                ),
                "mecanum_pub": self.create_publisher(
                    Twist, robot_namespace + "controller/cmd_vel", 1
                ),
            }
        ...
        if self.machine == "MentorPi_Mecanum":
            ...
            if steering_angle == 0:
                ...
                servo_state.position = [
                    1500 + int(math.degrees(-steering_angle) / 180 * 2000)
                ]
                data = SetPWMServoState()
                data.state = [servo_state]
                data.duration = 0.02
