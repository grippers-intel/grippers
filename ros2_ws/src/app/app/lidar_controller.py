...
from interfaces.srv import SetFloat64List, SetInt64
...
class LidarController(Node):
    ...
    def set_parameters_srv_callback(self, request, response):
        ...
        self.get_logger().info(
            f"\033[1;32mn_t:{new_threshold:2f}, n_a:{new_scan_angle:2f}, n_s:{new_speed:2f}\033[0m"
        )
        if not 0.3 <= new_threshold <= 1.5:
            response.success = False
            response.message = (
                f"New threshold ({new_threshold:.2f}) is out of range (0.3 ~ 1.5)"
            )
            return response
