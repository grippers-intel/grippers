...
OBJP = np.array(
    [[-1, -1, 0], [1, -1, 0], [-1, 1, 0], [1, 1, 0], [0, 0, 0]], dtype=np.float32
)
...
def draw_rectangle(img, imgpts):
    ...
    cv2.drawContours(
        img, [imgpts[:4]], -1, (0, 255, 0), -3
    )  # 绘制轮廓点，填充形式(draw contour points, filled)
    for i, j in zip(range(4), range(4, 8)):
        cv2.line(
            img, tuple(imgpts[i]), tuple(imgpts[j]), (255), 3
        )  # 绘制线连接点(draw points connected by lines)
    cv2.drawContours(
        img, [imgpts[4:]], -1, (0, 0, 255), 3
    )  # 绘制轮廓点，不填充(draw contour points, unfilled)
...
class ARNode(Node):
    def __init__(self, name):
        ...
        self.camera_intrinsic = np.matrix(
            [[619.063979, 0, 302.560920], [0, 613.745352, 237.714934], [0, 0, 1]]
        )
        self.dist_coeffs = np.array(
            [0.103085, -0.175586, -0.001190, -0.007046, 0.000000]
        )
        ...
        )  # 发布最终图像(publish the final image)
        self.create_service(
            Trigger, "~/enter", self.enter_srv_callback
        )  # 进入发玩法服务(enter the game service)
        self.create_service(
            Trigger, "~/exit", self.exit_srv_callback
        )  # 退出玩法服务(exit the game service)
        Heart(
            self,
            self.name + "/heartbeat",
            5,
        ...
        self.create_service(
            SetString, "~/set_model", self.set_model_srv_callback
        )  # 设置模型服务(set the model service)
        ...
        with self.lock:
            ...
            if self.camera_info_sub is None:
                self.camera_info_sub = self.create_subscription(
                    ...
                    1,
                )  # 订阅摄像头信息(subscribe to the camera information)
    ...
    def set_model_srv_callback(self, request, response):
        # 设置模型(set model)
        with self.lock:
            self.get_logger().info("\033[1;32m%s\033[0m" % f"set model {request.data}")
            if request.data == "":
                self.target_model = None
            else:
                self.target_model = request.data
                if (
                    self.target_model != "rectangle"
                ):  # 如果不是绘制立方体(if the cube is not being drawn)
                    # 加载模型(load the model)
                    obj = obj_load(
                        os.path.join(MODEL_PATH, self.target_model + ".obj"),
                        swapyz=True,
                    )
                    ...
                    for face in obj.faces:
                        ...
                        for vertex in face_vertices:
                            ...
                            if (
                                self.target_model != "cow"
                                and self.target_model != "wolf"
                            ):
                                colors.append(data[3:])
                        scale_matrix = (
                            np.array([[1, 0, 0], [0, 1, 0], [0, 0, 1]])
                            * MODELS_SCALE[self.target_model]
                        )  # 缩放(scale)
                        points = np.dot(np.array(points), scale_matrix)
                        if self.target_model == "bicycle":
                            points = np.array(
                                [[p[0] - 670, p[1] - 350, p[2]] for p in points]
                            )
                            points = R.from_euler(
                                "xyz", (0, 0, 180), degrees=True
                            ).apply(points)
                        elif self.target_model == "fox" or self.target_model == "chair":
    ...
    def image_proc(self, rgb_image, result_image):
        ...
        if self.target_model is not None:
            gray = cv2.cvtColor(
                rgb_image, cv2.COLOR_RGB2GRAY
            )  # 转为灰度图(convert into gray image)
            detections = self.tag_detector.detect(
                gray
            )  # aprilatg识别(aprilatg recognition)
            if detections != ():
                for detection in detections:  # 遍历(traverse)
                    ...
                    cv2.circle(
                        result_image, (int(lb[0]), int(lb[1])), 2, (0, 255, 255), -1
                    )
                    cv2.circle(
                        result_image, (int(lt[0]), int(lt[1])), 2, (0, 255, 255), -1
                    )
                    cv2.circle(
                        result_image, (int(rb[0]), int(rb[1])), 2, (0, 255, 255), -1
                    )
                    cv2.circle(
                        result_image, (int(rt[0]), int(rt[1])), 2, (0, 255, 255), -1
                    )
                    ...
                    ret, rvecs, tvecs = cv2.solvePnP(
                        OBJP, corners, self.camera_intrinsic, self.dist_coeffs
                    )
                    if (
                        self.target_model == "rectangle"
                    ):  # 如果要显示立方体则单独处理(if the cube needs to be displayed, process independently)
                        # 反向投影将世界坐标系点转换到图像点(backprojection converts world coordinate system points to image points)
                        imgpts, jac = cv2.projectPoints(
                            AXIS, rvecs, tvecs, self.camera_intrinsic, self.dist_coeffs
                        )
                        result_image = draw_rectangle(result_image, imgpts)
                    else:
                        for points, color in self.obj:
                            dst, jac = cv2.projectPoints(
                                points.reshape(-1, 1, 3) / 100.0,
                                rvecs,
                                tvecs,
