...
class ColorPicker:
    ...
    def __call__(self, image, result_image):
        ...
        l, a, b = (
            int(l / (2 * len(self.color))),
            int(a / (2 * len(self.color))),
            int(b / (2 * len(self.color))),
        )
        r, g, b_ = (
            int(r / (2 * len(self.rgb))),
            int(g / (2 * len(self.rgb))),
            int(b_ / (2 * len(self.rgb))),
        )
        if 0 <= x < w and 0 <= y < h:
            result_image = cv2.circle(
                result_image, (x, y), self.count, (r, g, b_), 2 * self.count
            )
            result_image = cv2.circle(
                result_image, (x, y), self.count, (255, 255, 0), 5
            )
        ...
        if len(self.rgb) / 2 > self.repeat:
            ...
            self.rgb.remove(self.rgb[0])
        self.count = min(self.count, self.repeat)
