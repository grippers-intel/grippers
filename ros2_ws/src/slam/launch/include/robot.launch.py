...
def launch_setup(context):
    ...
    depth_camera_name = LaunchConfiguration(
        "depth_camera_name", default="depth_cam"
    ).perform(context)
    ...
    map_frame = (
        f"{frame_prefix}map" if robot_name == master_name else f"{master_name}/map"
    )
    cmd_vel_topic = f"{topic_prefix}/controller/cmd_vel"
    scan_raw = f"{topic_prefix}/scan_raw"
    scan_topic = f"{topic_prefix}/scan_raw"
    odom_frame = f"{frame_prefix}odom"
    base_frame = f"{frame_prefix}base_footprint"
    lidar_frame = f"{frame_prefix}lidar_frame"
    imu_frame = f"{frame_prefix}imu_link"
    ...
    init_pose_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(controller_package_path, "launch/init_pose.launch.py")
        ),
        launch_arguments={
            "namespace": namespace,
            "use_namespace": use_namespace,
