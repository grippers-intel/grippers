...
def launch_setup(context):
    ...
    slam_method = LaunchConfiguration("slam_method", default="slam_toolbox").perform(
        context
    )
    sim = LaunchConfiguration("sim", default="false").perform(context)
    master_name = LaunchConfiguration(
        "master_name", default=os.environ.get("MASTER", "/")
    ).perform(context)
    robot_name = LaunchConfiguration(
        "robot_name", default=os.environ.get("HOST", "/")
    ).perform(context)
