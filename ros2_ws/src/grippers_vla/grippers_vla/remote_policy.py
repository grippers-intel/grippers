...
1. **Pi CPU 를 비운다.** 175ms 는 ROS2 스택이 안 떠 있을 때 잰 값이다. 시연
   중에는 perception(카메라 잡기 + yolov5)·arm_driver(시리얼)·미션 FSM 이
   같은 4코어를 나눠 쓴다.
2. **모델을 갈아끼우기 쉽다.** 지금은 체크포인트를 바꾸려면 200MB 를 Pi 로
   scp 해야 한다. 오프로드면 서버 쪽 파일만 바꾸면 된다.
...
#: 서버 응답을 이만큼 기다린다. Pi 로컬 추론이 175ms 이므로 왕복은 1초를 넘길
#: 일이 없지만, 서버가 CPU 로 돌거나 첫 호출이 걸릴 때를 보고 여유를 뒀다.
