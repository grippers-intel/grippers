...
class FingerControlNode(Node):
    def __init__(self, name):
        ...
        self.joints_pub = self.create_publisher(
            ServosPosition, "/servo_controller", 1
        )  # 舵机控制(servo control)
        ...
        self.kinematics_client = self.create_client(
            SetRobotPose, "/kinematics/set_pose_target", callback_group=timer_cb_group
        )
            ...
            if results is not None and results.multi_hand_landmarks:
                for hand_landmarks in results.multi_hand_landmarks:
                    self.drawing.draw_landmarks(
                        bgr_image, hand_landmarks, mp.solutions.hands.HAND_CONNECTIONS
                    )
                    ...
                    try:
                        ...
                        cv2.circle(
                            bgr_image,
                            (int(thumb_finger_tip[0]), int(thumb_finger_tip[1])),
                            10,
                            (0, 255, 255),
                            -1,
                        )
                        ...
                        d = math.sqrt(
                            math.pow(thumb_finger_tip[0] - index_finger_tip[0], 2)
                            + math.pow(thumb_finger_tip[1] - index_finger_tip[1], 2)
                        )
                        ...
                        if abs(d - self.last_d) > 10:
                            ...
                            self.z_dis = min(self.z_dis, 0.46)
                            self.z_dis = max(self.z_dis, 0.36)
                            msg = set_pose_target(
                                [self.x_init, 0.0, self.z_dis],
                                0.0,
                                [-180.0, 180.0],
                                1.0,
                            )
