...
class YoLov5TRT:
    ...
    def __init__(
        self, engine_file_path, plugin, classes, conf_thresh=0.8, iou_threshold=0.4
    ):
...
if __name__ == "__main__":
    ...
    yolov5_wrapper = YoLov5TRT(
        "./traffic_signs_640s_7_0.engine", "./libmyplugins.so", classes
    )
    # yolov5_wrapper = YoLov5TRT('./yolov5s.engine', './libmyplugins.so', classes)
    single_picture = False
    if single_picture:
        frame = cv2.imread(
            "/home/ubuntu/third_party_ros2/my_data/JPEGImages/image_1.jpg"
        )
