...
from interfaces.srv import SetCircleROI, SetColorDetectParam, SetLineROI
...
class ColorDetectNode(Node):
    def __init__(self, name):
        ...
        self.lab_data = common.get_yaml_data(
        ...
        self.line_roi = {
            "roi_up": line_roi["roi_up"],
            "roi_center": line_roi["roi_center"],
            "roi_down": line_roi["roi_down"],
        }
        circle_roi = self.get_parameters_by_prefix("roi_circle")
        self.circle_roi = {
            "x_min": circle_roi["x_min"].value,
            "x_max": circle_roi["x_max"].value,
            "y_min": circle_roi["y_min"].value,
            "y_max": circle_roi["y_max"].value,
        }
        rect_roi = self.get_parameters_by_prefix("roi_rect")
        self.rect_roi = {
            "x_min": rect_roi["x_min"].value,
            "x_max": rect_roi["x_max"].value,
            "y_min": rect_roi["y_min"].value,
            "y_max": rect_roi["y_max"].value,
        }
        ...
        self.image_sub = self.create_subscription(
            Image,
            "/%s/camera_publisher/rgb0/image" % self.camera,
            self.image_callback,
            1,
        ...
        self.create_service(
            SetColorDetectParam, "~/set_param", self.set_param_srv_callback
        )
        ...
        if len(request.data) == 1:
            ...
            for i in request.data:
                self.get_logger().info(
                    "\033[1;32m%s\033[0m" % str([i.color_name, i.detect_type])
                )
    ...
    def main(self):
        while self.running:
            ...
            )  # bgr转lab(convert from BGR to LAB)
            ...
            for i in self.target_colors:
                if self.detect_type[i] == "line" and i != "":
                    ...
                    for roi in self.line_roi:
                        ...
                        blob = img_blur[
                            roi_value[0] : roi_value[1], roi_value[2] : roi_value[3]
                        ]  # 截取roi(crop ROI)
                        mask = cv2.inRange(
                            blob,
                            tuple(self.lab_data["lab"][self.camera_type][i]["min"]),
                            tuple(self.lab_data["lab"][self.camera_type][i]["max"]),
                        )  # 二值化(binarization)
                        eroded = cv2.erode(
                        ...
                        contours = cv2.findContours(
                            dilated, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_TC89_L1
                        )[
                            -2
                        ]  # 找轮廓(find contours)
                        max_contour_area = common.get_area_max_contour(contours, 0)[
                            0
                        ]  # 获取最大面积对应轮廓(find the contour with the largest area)
                        if max_contour_area is not None:
                            rect = cv2.minAreaRect(
                                max_contour_area
                            )  # 最小外接矩形(find the minimum bounding rectangle)
                            box = np.intp(cv2.boxPoints(rect))  # 四个角(four corners)
                            for j in range(4):
                                box[j, 1] = box[j, 1] + roi_value[0]
                            cv2.drawContours(
                                result_image, [box], -1, common.range_rgb[i], 2
                            )  # 画出四个点组成的矩形(draw the rectangle composed of the four points)

                            # 获取矩形对角点(obtain the diagonal points of the rectangle)
                            pt1_x, pt1_y = box[0, 0], box[0, 1]
                            pt3_x, pt3_y = box[2, 0], box[2, 1]
                            # 线的中心点(the center point of the line)
                            line_center_x, line_center_y = (pt1_x + pt3_x) / 2, (
                                pt1_y + pt3_y
                            ) / 2
                    ...
                    if self.detect_type[i] == "rect":
                        if self.enable_roi_display:
                            cv2.rectangle(
                                result_image,
                                (self.rect_roi["x_min"], self.rect_roi["y_min"]),
                                (self.rect_roi["x_max"], self.rect_roi["y_max"]),
                                (0, 255, 255),
                                1,
                            )
                        blob = img_blur[
                            self.rect_roi["y_min"] : self.rect_roi["y_max"],
                            self.rect_roi["x_min"] : self.rect_roi["x_max"],
                        ]
                    else:
                        if self.enable_roi_display:
                            cv2.rectangle(
                                result_image,
                                (self.circle_roi["x_min"], self.circle_roi["y_min"]),
                                (self.circle_roi["x_max"], self.circle_roi["y_max"]),
                                (0, 255, 255),
                                1,
                            )
                        blob = img_blur[
                            self.circle_roi["y_min"] : self.circle_roi["y_max"],
                            self.circle_roi["x_min"] : self.circle_roi["x_max"],
                        ]
                    mask = cv2.inRange(
                        blob,
                        tuple(self.lab_data["lab"][self.camera_type][i]["min"]),
            ...
            )  # 按比重计算中心点(calculate the center point based on weighted average)
            ...
            if areaMaxContour_rect is not None:
                rect = cv2.minAreaRect(
                    areaMaxContour_rect
                )  # 最小外接矩形(the minimum bounding rectangle)
                box = np.intp(
                    cv2.boxPoints(rect)
                )  # 最小外接矩形的四个顶点(four corner points of the minimum bounding rectangle)
                for i in range(4):
                    box[i, 0] = box[i, 0] + self.rect_roi["x_min"]
                    box[i, 1] = box[i, 1] + self.rect_roi["y_min"]
                cv2.drawContours(
                    result_image, [box], -1, common.range_rgb[color_area_max_rect], 2
                )  # 画出四个点组成的矩形(draw the rectangle composed of the four points)
            ...
            if areaMaxContour_circle is not None:
                (x, y), radius = cv2.minEnclosingCircle(
                    areaMaxContour_circle
                )  # 获取最小外接圆(obtain the minimum circumscribed circle)
                ...
                cv2.circle(
                    ...
                    (x, y),
                    radius,
                    common.range_rgb[color_area_max_circle],
                    2,
                )  # 画圆(draw circle)
