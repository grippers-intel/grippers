...
def getAreaMaxContour(contours):
    ...
    for c in contours:  # 历遍所有轮廓(iterate through all contours)
        contour_area_temp = math.fabs(
            cv2.contourArea(c)
        )  # 计算轮廓面积(calculate the area of the contour)
        if contour_area_temp > contour_area_max:
            contour_area_max = contour_area_temp
            if (
                contour_area_temp > 50
            ):  # 只有在面积大于50时，最大面积的轮廓才是有效的，以过滤干扰(Only when the area is greater than 50, the contour with the largest area is valid. This allows to filter out interference)
                area_max_contour = c

    return (
        area_max_contour,
        contour_area_max,
    )  # 返回最大的轮廓(return the contour with the largest area)
...
def run(img):
    ...
    )  # 将图像转换到LAB空间(convert image to LAB space)
    ...
    for i in ["red", "green", "blue"]:
        ...
        )  # 对原图像和掩模进行位运算(perform bitwise operation on the original image and the mask)
        eroded = cv2.erode(
            frame_mask, cv2.getStructuringElement(cv2.MORPH_RECT, (3, 3))
        )  # 腐蚀(erode)
        dilated = cv2.dilate(
    ...
    if max_area > 200:  # 有找到最大面积(the maximum area is found)
        ...
        cv2.circle(
            img, (centerX, centerY), radius, range_rgb[color_area_max], 2
        )  # 画圆(draw circle)
