    ...
    def set_color_srv_callback(self, request, response):
        ...
        if res.success:
            self.get_logger().info(
                "\033[1;32m%s\033[0m" % "start_track_%s" % msg_red.color_name
            )
        else:
    ...
    def main(self):
        while self.running:
            if self.center is not None and self.start:
                ...
                self.y_dis = max(self.y_dis, 800)
                self.y_dis = min(self.y_dis, 1900)
                self.pid_z.SetPoint = self.center.height / 2
                self.pid_z.update(self.center.y)
                self.z_dis -= self.pid_z.output
                self.z_dis = min(self.z_dis, 1900)
                self.z_dis = max(self.z_dis, 800)
