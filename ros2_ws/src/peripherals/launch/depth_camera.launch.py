...
def generate_launch_description():
    ...
    depth_camera_name_arg = DeclareLaunchArgument(
        "depth_camera_name", default_value="depth_cam"
    )

    tf_prefix_arg = DeclareLaunchArgument("tf_prefix", default_value="")

    app_arg = DeclareLaunchArgument("app", default_value="false")
    ...
    if camera_type == "ascamera":
        camera_launch = IncludeLaunchDescription(
            PythonLaunchDescriptionSource(
                os.path.join(
                    peripherals_package_path, "launch/include/ascamera.launch.py"
                )
            ),
            launch_arguments={
        ...
        camera_launch = GroupAction(
            [
                IncludeLaunchDescription(
                    PythonLaunchDescriptionSource(
                        os.path.join(
                            peripherals_package_path, "launch/usb_cam.launch.py"
                        )
                    )
                ),
                Node(
    ...
    return LaunchDescription(
        [depth_camera_name_arg, tf_prefix_arg, app_arg, camera_launch]
    )
