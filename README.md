...
- **설치 높이 1.6 m (8/14 확정)** — 근거는 **Houdini 합성 데이터가 이 높이로 생성 중**이라는 것 하나입니다. 천장 · 거치 물리 제약에서 나온 값이 아니므로, 별도 제약이 있다면 기록해 주세요
- ⚠️ **1.6 m 의 대가는 가림입니다.** 먼 모서리 고도각이 작업 공간에 따라 **24~30°** 까지 떨어집니다 (2.5×2.5 m → 24°, 2×2 m → 30°). 로봇(0.3 m) · 상자(0.4 m) 가 시야를 가리므로 **밀집 · 겹침 배치를 피하고 A2 실측에 가림 케이스를 반드시 포함**할 것
