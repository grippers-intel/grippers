...
| 📷 | **단안 인식** — 모서리 웹캠 **2대** + 바닥면 호모그래피. 깊이 카메라 없음 | [perception](docs/subsystems/perception.md) |
...
| `perception` | 로봇 | **웹캠 3대 소유** (모서리 2 + 탑재 1), 바닥면 호모그래피 2장, 두 시야 병합, 상자 색 탐색, 클리어런스 | 상시 구동 + 디바이스 소유 | 김동혁 · 김희수 |
...
| **모서리 웹캠 ×2** | **Logitech C270 HD** (720p · 대각 55° · 고정초점) · **마주보는 두 모서리 · 높이 1.6 m · 대각선 바깥 1.90 m 후퇴 (8/18 확정)** | 발주 |
| **로봇 탑재 웹캠** | USB — 근거리 파지 확인 · 클리어런스 | 발주 |
| **웹캠 거치 수단** | **환경 고정물에 1.6 m 로 거치 (8/17 확정)** — 삼각대 · 폴대 · 클램프 발주 없음 | ✅ 확정 |
| **설치 공간** | 대각선 방향 **6.63 m** · 바닥 약 **4.7 × 4.7 m** — C270 후퇴 배치의 선행 조건 | 🔴 확인 필요 |
...
- **모서리 웹캠은 C270 2대 (8/18 확정)** — 720p 는 문제가 아닙니다. 화각이 좁아 픽셀 밀도는 1080p·65° 대비 6% 손해뿐입니다. **문제는 HFOV 48.8° 로 2×2 m 를 한 대가 담지 못한다는 것**이고, 그래서 2대로 절반씩 나눕니다 → [perception](docs/subsystems/perception.md)
- 🔴 **설치 공간 6.63 m 확보가 선행 조건** — 각 카메라를 모서리 바깥으로 1.90 m 물려야 화각이 작업 공간을 담습니다. 이 여유가 안 나오면 배치가 성립하지 않으므로 **발주 전에 실측**할 것 (미결 #19)
...
- ⚠️ **후퇴 배치의 대가는 고도각입니다.** 최악점(옆 모서리) 고도각이 **23.9°** (슬랜트 3.95 m) 로, 단독 구성 계획의 30° 보다 낮습니다. 10 cm 물체 중심점 오차가 **225 mm** 이므로 **호모그래피 입력은 반드시 접지선**을 쓸 것
- ✅ **가림은 2대 구성으로 상당 부분 해소됩니다** — 서로 반대편에서 보므로 한쪽이 가려도 다른 쪽이 봅니다. 다만 **두 카메라가 동시에 가리는 배치**는 여전히 위험하므로 A2 실측에 포함할 것
- ✅ **C270 은 고정초점이라 AF 고정 문제가 원천 해소됩니다** (이슈 #89). **AWB · AE 는 여전히 `v4l2-ctl` 로 꺼야 합니다**
...
│   │   ├── perception.md       #   인식 — C270 2대 배치 · 화각 · 호모그래피 · 가림
