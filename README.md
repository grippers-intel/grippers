...
```
  ...
  🟢🔵 상자   먼 쪽 양쪽 모서리 · 벽 이격 0.05 · 상자 간 0.90 m
  ▪⬡■ 가베    원기둥 · 정육면체 · 육각기둥 (눕혀서 배치)
  ♞   체스말   나이트 · 퀸 · 룩 — 3D 프린팅
...
| 고도각 | 29.7 ~ 51.6° |
| 최악점 | **1.87 mm/px** — 55 mm 물체가 **29.4 px**, 40 mm 가 21.4 px |
| 실효 커버 시작 | 0.21 m (울타리 0.25 기준) |

> [!IMPORTANT]
> **"작업 공간 네 모서리가 한 화면에" 는 이 기하에서 성립하지 않습니다** — 가로 7% 초과입니다.
> 2대 대향이라 근거리 모서리는 맞은편 카메라가 담당하고 합집합으로 전 구역이 덮이므로 운용에는 문제가 없지만,
> **A2 호모그래피 기지점을 네 모서리 ArUco 로 잡을 계획이면 후퇴 1.20 m (바닥 4.20 m) 가 필요합니다.**
> 카메라별로 기지점을 따로 두면 0.95 m 를 유지합니다 — A2 담당과 맞출 항목입니다.
...
>
> [!WARNING]
> **배치도(8/20)는 상자를 🟢🔵 2개만 그리고 있습니다.** ⚫🔴 방해 선택지의 배치 위치가 도면에 없습니다.
> **2개로 확정되면 무작위 기준선이 25% → 50% 로 올라가 이 지표의 설득력이 절반이 됩니다.** 최종 상자 개수를 확정해야 하는 항목입니다 → [`workspace_layout.html`](docs/design/workspace_layout.html)
...
| **상자** | ⚫🔴🔵🟢 · 바닥 0.40 × 0.40 m · **검정은 밝은 테두리 필수** · 🟢🔵 는 먼 쪽 양쪽 모서리(벽 이격 0.05 · 간격 0.90 m) | 발주 |
