...
| **모서리 웹캠** | USB **1080p 이상 · HFOV 60~70°** · **설치 높이 1.6 m (8/14 확정)** | 발주 |
| **로봇 탑재 웹캠** | USB — 근거리 파지 확인 · 클리어런스 | 발주 |
| **웹캠 거치 수단** | 1.6 m 고정 — 삼각대 / 폴대 / 클램프 중 택 1 | 🔴 결정 필요 |
| **상자 4개** | ⚫🔴🔵🟢 · 입구 짧은 변 0.40 m · **검정은 밝은 테두리 필수** | 발주 |
| **가베 교구** | **한 변 5 cm 이상** · 목재 경량 · **원기둥 · 정육면체 · 육각기둥 3종** | 발주 |
...
- **설치 높이 1.6 m** — Houdini 합성 데이터가 이 높이로 생성 중이라 실기를 여기에 맞춥니다. 거리가 짧아 해상도는 유리하지만 **먼 모서리 고도각이 약 30°로 낮아져 가림이 커집니다** → 밀집 · 겹침 배치를 피할 것
- **가베 5 cm 이상** — 기본형 3~5 cm 는 검출 하한 미달. **시중 기본 세트는 대부분 미달**이므로 대형 가베 · 유아용 원목 블록 쪽을 보고 **구매 전 실측 치수를 확인**할 것
