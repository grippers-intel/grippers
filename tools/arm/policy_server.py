...
## 사용법
    ...
    DP 체크포인트가 안 읽힌다(0.6.x processor 형식):
        ...
        .venv-dp/Scripts/python.exe grippers/tools/arm/policy_server.py
          ...
          --device cuda --scheduler DDPM --denoise 10 --n-action-steps 63
          --host 0.0.0.0

    ⚠️ 이 인자 묶음은 tools/run_vla_mission.sh 가 못 붙었을 때 뱉는 안내문과
    **같아야 한다.** 그 스크립트는 /health 로 n_action_steps 를 되읽어
    EXPECT_N_ACTION_STEPS(63) 와 맞는지도 본다.

    ⚠️ 예전에 여기 DDIM 25 라고 적혀 있었다. DDIM 은 쓰지 말 것 — 이 모델에서는
    같은 시간에 오차가 두 배다(policy_runner 주석, --scheduler 도움말).

    Pi 쪽 — 손으로 치지 말고 tools/run_vla_mission.sh 를 쓸 것. 인자를
    빠뜨리면 ros2 launch 가 조용히 무시한다(그 스크립트 주석의 사고 두 건):

        docker exec IntelPi /grippers/tools/run_vla_mission.sh --host-ip <노트북 IP>
...
def main() -> None:
    ...
    ap.add_argument("--scheduler", default=None, choices=("DDPM", "DDIM"),
                    help="DP 노이즈 스케줄러. ⚠️ DDIM 을 쓰지 말 것 — 이 모델에서는"
                         " 같은 시간에 오차가 두 배다(policy_runner 주석). 재 볼 때만")
    # ⚠️ 청크 길이는 파지 성공을 좌우한다. 2026-09-06 실기:
    #
    # DP 기본 n_action_steps=32 는 청크가 1.07초인데, 학습 데이터에서 팔을
    # 뻗은 자세를 빠져나오는 데 3.0~6.5초가 걸린다(리눅스 세션이 v5_all
    # 68,365 프레임 최근접 검색으로 확인). 1.07초를 재생해도 거의 안 움직이니
    # 같은 관측 -> 같은 청크가 되고, 팔이 21초 동안 멈춘 채 그리퍼를 열고만
    # 있었다.
    #
    # 이건 2026-09-02 에 ACT 를 n_action_steps=30 으로 줄였다가 겪은 것과
    # **같은 고장**이다(0~29 무한 반복, 25초 정지). DP 라서가 아니라 청크가
    # 짧아서다 — 30 과 32 는 거의 같은 길이다.
    #
    # DP 의 상한은 horizon - n_obs_steps + 1 = 64 - 2 + 1 = 63 이다.
    # 63 이면 청크 2.10초로 두 배가 되고 듀티는 절반이 된다.
    ap.add_argument("--n-action-steps", type=int, default=None,
                    help="청크 하나의 스텝 수. DP 기본 32(1.07초)는 짧아서 "
                         "뻗은 자세에서 못 빠져나온다 — 63(2.10초) 권장. "
                         "상한은 horizon - n_obs_steps + 1")
    args = ap.parse_args()
