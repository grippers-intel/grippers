...
```bash
cd tools/a2
pip install opencv-python numpy          # 실측 도구 (a2_homography.py · usb_coexist.py)
pip install matplotlib                   # 배치 계산 도구 (coverage_analysis.py, §6)
```

`coverage_analysis.py` 는 그림을 그리므로 `matplotlib` 이 추가로 필요하다.
한글 폰트(`fonts-nanum`)가 없으면 그림의 글자가 깨진다 — 스크립트가 먼저 경고한다.
...
좌표는 A 가 `(900, -950)`, B 가 `(900, 2750)`, 높이 1650 mm 다.
...
python a2_homography.py calib --cam-id A --height 1650 --setback 950 --side 1800
```

`--height` · `--setback` · `--side` 는 **계획값이 아니라 그날 줄자로 잰 값**을 넣는다.
기본값은 계획값(1650 / 1400 / 1800)이라 안 넘기면 그대로 기록된다.

**호모그래피 자체는 이 값에 의존하지 않는다.** 사상은 바닥에 표시한 기지점 4개에서 나오므로,
카메라가 어디에 얼마나 높이 서 있든 잔차로 판정된다. 이 인자들은 **기록용**이다 — 남기지 않으면
나중에 "그날 어떻게 세웠길래 이 숫자가 나왔나" 를 복원할 수 없다. `report` 는 저장된 값을 쓰고,
실측이 안 들어온 항목에는 **(계획값 — 실측 미기입)** 을 붙인다.

> **프레이밍이 되는지는 계산이 아니라 여기서 판정된다.** `calib` 이 기지점을 클릭하라고 하는데
> 담당 절반의 귀퉁이가 화면 밖이면 클릭할 수 없다. **클릭이 안 되는 그 순간이 답이다** —
> 그때 후퇴 · 한 변 · 높이 중 무엇을 조정할지 정하면 된다. 미리 정해두지 않는다.
...
C270 ×2 · **마주보는 두 변 중앙 · 높이 1650 mm · 후퇴 950 mm · 하향 44.1°** ·
작업 공간 1.8 × 1.8 m · `f = 1410 px` 기준이다.
(배치도 Rev.I 8/20 팀 확정. 표는 `python tools/a2/coverage_analysis.py` 로 언제든 다시 만든다.)

> [!IMPORTANT]
> **높이 1650 은 삼각대 최대치라 고정값이다.** 조절할 수 있는 건 후퇴뿐이고,
> 후퇴를 줄이면 가로가 잘린다(최소 865 mm). 950 은 가로에 9.8% 여유를 둔 값이다.
> 단, 이 하한은 **가까운 벽 앞 0.26 m 를 포기한다는 전제** 위에 서 있다 —
> 그 앞은 울타리에 가려 어차피 안 찍히고, 마주보는 카메라가 덮는다.

`σmax` 는 이미지→월드 사상 야코비안의 **최대 특이값**, 즉 클릭 1 px 이 최악 방향으로
어긋났을 때의 월드 오차다. 괄호 안은 해석 근사 `slant / (f · sin θ)` 다.

표는 **커버 시작선(벽에서 0.26 m)부터** 잡는다. 그보다 앞은 울타리에 가리고 프레임
아래로도 잘려(`v = 403 / 360`) 애초에 안 찍히므로, 거기 수치를 올리면 없는 성능을
보고하게 된다. 그 구간은 마주보는 카메라가 덮는다.

| 지점 | 슬랜트 | 고도각 | **σmax (mm/px)** | 3 px 최악 | 30 mm 물체 | 40 mm 물체 |
|---|---|---|---|---|---|---|
| 커버 시작 변 중앙 | 2.04 m | 53.8° | **1.74** (1.80) | 5.2 mm | 20.7 px | 27.6 px |
| 커버 시작 모서리 | 2.23 m | 47.6° | **1.93** (2.14) | 5.8 mm | 18.9 px | 25.3 px |
| 담당 구역 중앙 | 2.16 m | 49.7° | **1.99** (2.01) | 6.0 mm | 19.5 px | 26.1 px |
| 경계 중앙 | 2.48 m | 41.7° | **2.64** (2.64) | 7.9 mm | 17.1 px | 22.8 px |
| **경계 모서리 (최악점)** | **2.64 m** | **38.7°** | **2.78** (2.99) | **8.3 mm** | **16.0 px** | **21.4 px** |

프레이밍은 `max|u| = 630 / 640`, `max|v| = 242 / 360` 이다. 가로가 빠듯한 쪽인데,
**카메라가 폭 1800 mm 를 통째로 담아야 하기 때문**이다. 이게 이 배치의 지배 제약이다.

> [!CAUTION]
> **좌우를 정확히 변의 중앙(x = 900)에 두어야 한다.** 후퇴 950 에서 허용 오차는
> **±14 mm 뿐이다** — 25 mm 어긋나면 `max|u|` 가 630 → 648, 50 mm 면 665,
> 100 mm 면 700 으로 **프레임을 넘는다.** 높이보다 이쪽이 훨씬 예민하다.
> 후퇴 1400 시절의 여유(50 mm 까지 무사)는 더 이상 없다. 줄자로 재서 세운다.

> [!CAUTION]
> **30 mm 물체는 최악점에서 16.0 px 로, 검출 하한 20 px 에 못 미친다.**
> 배치로는 못 고친다 — 전수 탐색해도 최대 17.7 px 다(`fig1_placement_sweep.png`).
> 20 px 을 보장하는 최소 폭은 **37 mm** 이고, **40 mm 로 확정**했다(21.4 px).
> 상한은 그리퍼다 — 40 mm 를 넘으면 파지가 안 된다. 여유가 7% 뿐이니
> 조명·모션블러로 유효 픽셀이 줄면 곧바로 하한이다. → #149 D1 · #180

> [!NOTE]
> 위치 정확도는 최악점 3 px 이 **8.3 mm** 로 기준 20 mm 안에 넉넉히 들어온다.
> 후퇴를 1400 → 950 으로 당기면서 슬랜트가 짧아진 덕이다.
> 높이 1900 안(#149 D3)은 이제 이득이 없다 — σmax 는 2.78 → 2.64 로 조금 나아지지만
> 40 mm 물체가 21.4 → **20.4 px** 로 떨어져 검출 하한에 붙는다. 1650 을 유지한다.
...
| 그림 | 무엇을 보이는가 |
|---|---|
| ![배치 전수 탐색](../../docs/assets/vision/fig1_placement_sweep.png) | 30 mm 는 어느 배치에서도 20 px 에 못 닿는다 — FOV 를 만족하는 **2547 조합** 중 최대 17.7 px. 주황 점선이 가벽 가림 띠다 |
| ![물체 크기](../../docs/assets/vision/fig2_object_size.png) | 확정 배치에서 물체 폭 37 mm 가 검출 하한과 만나는 지점 (확정 규격은 40 mm) |
...
네 변에 4대를 두어 각자 450 mm 띠만 맡겨도 30 mm 는 **18.7 px** 로 여전히 하한 아래다
(`four_edge_best()`, 실행하면 `[D6]` 줄로 찍힌다). D6(4대 안) 철회 근거가 이것이다.

**③ 가벽 가림은 제약이 아니라 비용이다.**
탐색에서 거르지 않고 등고선으로 겹쳐 그린다. 낮게 달수록 픽셀은 좋아지고 가림은 나빠진다.
하드 제약은 FOV 뿐이다 — 예전에 가림 204 mm 를 필터로 걸었는데, 그 값은 폐기된 모서리
배치에서 우연히 나온 수치였고 확정 배치(256 mm)를 오히려 제외해 버렸다.

**④ 높이 1650 은 삼각대 한계라 고정이고, 그 대가가 가림이다.**
가벽 가림 띠는 확정 리그에서 **256 mm** 다. 상자 뒤 그림자는 실물 치수가 나왔으므로
배치도 Rev.II 를 따른다 — **웹캠 1 쪽 0.35 m · 웹캠 2 쪽 0.18 m**(#180).
거치를 1900 mm 로 올리면 가림 띠는 203 mm 로 줄고 σmax 도 2.78 → 2.64 가 되지만,
**40 mm 물체가 21.4 → 20.4 px 로 떨어져** 검출 하한에 붙는다. 후퇴를 950 으로 당긴
뒤로는 이득이 없는 맞바꿈이라 1650 을 유지한다(#149 D3).

> [!NOTE]
> **재현성.** 표준출력 수치는 결정적이라 어디서 돌려도 같다 — **근거로 삼을 것은 그쪽**이다.
> PNG 는 matplotlib 버전과 폰트에 따라 바이트가 달라질 수 있으므로,
> 값이 바뀌지 않았는데 그림만 modified 로 보이면 툴체인 차이다.
