...
> 고도각이 낮을수록 이 오차가 커진다 — 확정 배치의 최악점에서 1 px 이 4.0 mm 다(§4).
...
> [!IMPORTANT]
> **네 모서리를 다 쓰려고 하지 마라.** 확정 배치(높이 2050 / 후퇴 1400)에서 담당 절반을
> 가장 크게 잡는 피치로 세우면 먼 모서리는 v = −487 로 **프레임 밖**이다.
> 네 모서리를 억지로 담으려고 피치를 올리면 max|v| 가 356 까지 차서 한계 360 에
> 여유가 1% 밖에 안 남는다 — 틸트가 1° 어긋나면 그대로 잘린다.

기지점은 **자기 절반의 세 모서리 + 담당 구역 안에 테이프로 찍은 점 하나**로 잡는다.
월드 프레임을 묶는 것은 "네 점을 공유하는 것"이 아니라 **두 카메라가 같은 줄자 원점과
...
C270 ×2 · **높이 2050 mm · 대각 후퇴 1400 mm** · 작업 공간 1.8 × 1.8 m · `f = 1411 px` 기준이다.
(#149 D3 확정값. 표는 `python tools/a2/coverage_analysis.py` 로 언제든 다시 만든다.)

`σmax` 는 이미지→월드 사상 야코비안의 **최대 특이값**, 즉 클릭 1 px 이 최악 방향으로
어긋났을 때의 월드 오차다. 괄호 안은 해석 근사 `slant / (f · sin θ)` 로, 최대 11% 보수적이다.

| 지점 | 슬랜트 | 고도각 | **σmax (mm/px)** | 3 px 최악 | 30 mm 물체 | 55 mm 물체 |
|---|---|---|---|---|---|---|
| 가까운 모서리 | 2.48 m | 55.7° | **2.08** (2.13) | 6.2 mm | 17.1 px | 31.3 px |
| 작업 공간 중앙 | 3.37 m | 37.5° | **3.83** (3.92) | 11.5 mm | 12.6 px | 23.0 px |
| **옆 모서리 (최악점)** | **3.60 m** | **34.7°** | **4.02** (4.48) | **12.1 mm** | **11.8 px** | **21.6 px** |
| 먼 모서리 *(상대 카메라 담당)* | 4.45 m | 27.5° | **6.11** (6.83) | 18.3 mm | 9.5 px | 17.5 px |

> [!CAUTION]
> **30 mm 물체는 최악점에서 11.8 px 로, 검출 하한 20 px 에 못 미친다.**
> 이건 배치로 못 고친다 — 높이 900~2150 mm × 후퇴 500~1850 mm 를 전수 탐색해도
> 30 mm 의 최대치는 13.6 px 다(`fig1_placement_sweep.png`).
> 최악점까지 20 px 을 보장하려면 **물체 폭이 51 mm 이상**이어야 한다. → #149 D1

> [!NOTE]
> 위치 정확도(σmax)는 확정 배치에서 여유가 있다. 최악점 3 px 이 12.1 mm 로 기준 20 mm 안이다.
> 병목은 정확도가 아니라 **물체가 화면에서 너무 작다는 것**이다. 둘을 섞어서 보면 안 된다.

> [!NOTE]
> 먼 모서리는 애초에 프레임 밖이다. **여기는 상대 카메라의 가까운 모서리이므로 그쪽이 담당한다**
> — 두 대를 마주 보게 놓은 이유가 이것이다.
> 담당 경계(대각선) 바깥 점을 자기 카메라로 풀려고 하면 안 된다.
...
| 그림 | 무엇을 보이는가 |
|---|---|
| ![배치 전수 탐색](../../docs/assets/vision/fig1_placement_sweep.png) | 높이·후퇴를 어떻게 잡아도 30 mm 는 20 px 에 못 닿는다 (탐색 1556 조합, 최대 13.6 px) |
| ![물체 크기](../../docs/assets/vision/fig2_object_size.png) | 확정 배치에서 물체 폭 51 mm 가 검출 하한과 만나는 지점 |
| ![2대 vs 4대](../../docs/assets/vision/fig3_two_vs_four.png) | 담당 면적을 1/4 로 줄이면 30 mm 도 22.2 px — 대신 카메라 2대 추가 |

읽는 법 하나만 짚어 둔다. **검출(픽셀 폭)과 정확도(σmax)는 서로 다른 병목이다.**
확정 배치는 정확도 쪽으로 최적화한 값이고 그쪽은 여유가 있다(최악점 3 px = 12.1 mm < 20 mm).
픽셀 폭을 조금이라도 더 벌겠다고 배치를 옮기면 11.8 → 최대 13.6 px 을 얻는 대신
σmax 가 4.02 → 6.2 mm/px 으로 무너진다. **얻는 것보다 잃는 것이 크다.**
