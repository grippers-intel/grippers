...
> 고도각이 낮을수록 이 오차가 커진다 — 권고 배치의 최악점에서 1 px 이 2.24 mm 다(§4).
...
**카메라는 마주보는 두 "변 중앙" 바깥에 선다.** 모서리가 아니다 —
모서리에 두면 담당 구역이 삼각형이 되어 최악점이 3.60 m 로 멀어지고,
변 중앙에 두면 납작한 직사각형이라 2.69 m 로 가까워진다(§4·§6).
...
C270 ×2 · **마주보는 두 변 중앙 · 높이 2050 mm · 후퇴 600 mm · 하향 63.8°** ·
작업 공간 1.8 × 1.8 m · `f = 1411 px` 기준이다.
(#149 D3. 표는 `python tools/a2/coverage_analysis.py` 로 언제든 다시 만든다.)

`σmax` 는 이미지→월드 사상 야코비안의 **최대 특이값**, 즉 클릭 1 px 이 최악 방향으로
어긋났을 때의 월드 오차다. 괄호 안은 해석 근사 `slant / (f · sin θ)` 로, 최대 12% 보수적이다.

| 지점 | 슬랜트 | 고도각 | **σmax (mm/px)** | 3 px 최악 | 30 mm 물체 | 45 mm 물체 |
|---|---|---|---|---|---|---|
| 가까운 변 중앙 | 2.14 m | 73.7° | **1.53** (1.58) | 4.6 mm | 19.8 px | 29.7 px |
| 가까운 변 모서리 | 2.32 m | 62.2° | **1.66** (1.86) | 5.0 mm | 18.3 px | 27.4 px |
| 담당 구역 중앙 | 2.30 m | 62.9° | **1.83** (1.83) | 5.5 mm | 18.4 px | 27.6 px |
| 경계 중앙 | 2.54 m | 53.8° | **2.16** (2.23) | 6.5 mm | 16.7 px | 25.0 px |
| **경계 모서리 (최악점)** | **2.69 m** | **49.5°** | **2.24** (2.51) | **6.7 mm** | **15.7 px** | **23.6 px** |

프레이밍은 `max|u| = 604 / 640`, `max|v| = 247 / 360` 이다. 가로가 빠듯한 쪽인데,
**카메라가 폭 1800 mm 를 통째로 담아야 하기 때문**이다. 이게 이 배치의 지배 제약이다.

> [!CAUTION]
> **30 mm 물체는 최악점에서 15.7 px 로, 검출 하한 20 px 에 못 미친다.**
> 배치로는 못 고친다 — 높이 900~2600 mm × 후퇴 100~1600 mm 를 전수 탐색해도
> 30 mm 의 최대치는 17.4 px 다(`fig1_placement_sweep.png`).
> 최악점까지 20 px 을 보장하려면 **물체 폭이 38 mm 이상**이어야 한다.
> 여유를 보아 **45 mm** 를 권고한다(23.6 px). → #149 D1

> [!NOTE]
> 위치 정확도(σmax)는 이 배치에서 여유가 크다. 최악점 3 px 이 6.7 mm 로 기준 20 mm 의 1/3 이다.
> 병목은 정확도가 아니라 **물체가 화면에서 너무 작다는 것**이다. 둘을 섞어서 보면 안 된다.

> [!NOTE]
> **왜 두 대인가.** 한 대로 1.8 × 1.8 전체를 담는 것 자체는 가능하다(예: 높이 2050 · 후퇴 1200,
> max|u| 548 · max|v| 317). 그러나 그때 먼 모서리는 슬랜트 3.74 m 로 30 mm 가 11.3 px 이 되고
> 가림도 한 방향에서만 생긴다. **화각이 아니라 해상도와 가림 때문에** 두 대를 쓴다.
...
| ![물체 크기](../../docs/assets/vision/fig2_object_size.png) | 권고 배치에서 물체 폭 38 mm 가 검출 하한과 만나는 지점 |
| ![배치 비교](../../docs/assets/vision/fig3_layout_compare.png) | 모서리 배치와 변 중앙 배치의 차이 — 최악점이 3.60 m 에서 2.69 m 로 |

읽는 법 둘을 짚어 둔다.

**① 검출(픽셀 폭)과 정확도(σmax)는 서로 다른 병목이다.**
이 배치는 정확도 쪽으로 여유가 크다(최악점 3 px = 6.7 mm ≪ 20 mm).
남은 문제는 물체가 작다는 것 하나뿐이고, 그건 물체를 키워야 풀린다.

**② 지배 제약은 "한 대가 폭 1800 mm 를 통째로 담아야 한다"는 것이다.**
그래서 카메라를 몇 대로 늘려도 가로 해상도는 거의 안 좋아진다 —
네 변에 4대를 두어도 30 mm 는 18.7 px 로 여전히 하한 아래다.
가로를 쪼개지 않는 한 대수 증설은 답이 아니다.
