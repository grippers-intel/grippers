...
# 한 번의 보정 명령을 내는 시간. 짧을수록 오버슈트가 적지만 정지마찰 때문에
# 너무 짧으면 아예 안 움직인다.
ALIGN_BURST_S = 0.30
...
class AutoGraspNode(GraspTestNode):
    ...
    def turn_burst(self, angular_z, seconds=ALIGN_BURST_S):
        twist = Twist()
        twist.angular.z = angular_z
        self._burst(self.turn_pub, twist, seconds)

    def drive_burst(self, linear_x, seconds=ALIGN_BURST_S):
        twist = Twist()
...
def align(node, raw_cls, log) -> bool:
    """물체를 카메라 정면 TARGET_FORWARD_M에 놓는다. 성공하면 True.
    ...
    의미가 있으므로 좌우를 우선한다."""
    print(f"\n[1단계] 정렬 — 목표: 전방 {TARGET_FORWARD_M*100:.0f}cm, 좌우 0cm")
    lost = 0
    turn_stall = 0
    last_x = None
    ...
    for iteration in range(1, ALIGN_MAX_ITERATIONS + 1):
        ...
        if abs(lateral_m) > LATERAL_TOL_M:
            ...
            if last_x is not None and abs(obs_x - last_x) < TURN_PROGRESS_PX:
                turn_stall += 1
                if turn_stall >= TURN_STALL_LIMIT:
                    print(
                        f"  ⚠️ 회전 명령을 {turn_stall}번 냈는데 화면상 물체가 "
                        f"{TURN_PROGRESS_PX}px도 안 움직였습니다 — "
                        f"{ALIGN_TURN_RAD_S} rad/s가 정지마찰을 못 이기는 것으로 "
                        "보입니다. ALIGN_TURN_RAD_S를 0.3 이상으로 올리세요 "
                        "(실측으로 도는 것이 확인된 최저값)."
                    )
                ...
                turn_stall = 0
            last_x = obs_x
            node.turn_burst(-ALIGN_TURN_RAD_S if lateral_m > 0 else ALIGN_TURN_RAD_S)
            time.sleep(ALIGN_SETTLE_S)
            continue
...
def main():
    ...
    try:
        ...
        if not align(node, args.raw_cls, log):
            return 2
