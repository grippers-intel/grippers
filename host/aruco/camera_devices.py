...
def list_video_devices() -> list[tuple[int, str, str]]:
    ...
    Windows 는 DirectShow 를 직접 열거한다 — 그 순서가 곧 cv2 인덱스라
    이름으로 카메라를 고를 수 있다. macOS 는 `camera_backend` 가
    system_profiler 로 읽는다. ⚠️ macOS 에서는 그 순서가 cv2 인덱스와
...
def open_camera(index: int) -> "cv2.VideoCapture":
    ...
    warning = _backend.lock_focus(cap, getattr(cfg, "CAM_FOCUS", None))
    if warning and not _focus_warned:
        # 한 번만 찍는다. 카메라 두 대를 여는 호출부가 있어서 매번 찍으면
        # 사람이 무시하게 된다.
        print(f"[camera] {warning}", file=sys.stderr)
        globals()["_focus_warned"] = True
    return cap
