...
# 3cm 그대로 뒀다.
#
# 2026-09-05 밤 다시 넓힌다(사용자 지시 — "충분히 들어갈 수 있는데 굳이
# servo1을 돌리는 경우가 있네? servo1을 안돌려도 되도록 바구니 내
# 투하영역을 키우자"). check_basket_insert_gate()가 PLACE에서 매 사이클
# 다시 재는 facing_error_deg(=servo1 보정 각도)는 이 목표영역 사각형의
# "가장 가까운 점" 방위로 계산되므로, 사각형이 넓을수록 같은 로봇
# 자세에서도 필요한 보정이 작아진다(실기: FACE_BOX 허용각을 45도로
# 넓힌 뒤 stop 위치의 잔차가 커져 +21.3도·+22.4도·-5.5도 보정이 매번
# 걸렸다). 물리 바구니 폭(config.BOX_W=0.210m)에서 벽 여유를 남기며
# 가로를 5cm->6.5cm(양쪽 13cm, 벽까지 4cm 여유), 깊이는 물리 여유가
# 훨씬 크므로(BOX_L=0.350m) 3cm->6cm로 넓혔다.
#
# ⚠️ 이 값은 좌우 폭 이력 전체가 실기 사고(2026-09-03/04)로 오르내린
# 자리다 — 이번 값도 실기로 검증된 적이 없다. 첫 시험에서 벽 쪽으로
# 붙어 떨어지면 이 값부터 의심하고 되돌릴 것.
TARGET_HALF_WIDTH_M = 0.065      # 입구 중심 기준 좌우(가로) 절반폭
TARGET_INSET_DEPTH_M = 0.06    # 입구 중심에서 바구니 안쪽으로 깊이
